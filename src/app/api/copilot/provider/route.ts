...
const KINDS: CopilotProviderKind[] = ["env", "openai-key", "chatgpt-oauth"];
...
export async function PUT(request: NextRequest): Promise<NextResponse> {
  ...
  const patch: { provider?: CopilotProviderKind; model?: string; apiKey?: string } = {};
  ...
  audit({
    ...
    newValue: JSON.stringify({ provider: saved.provider, model: saved.model || defaultModelFor(saved.provider), apiKey: saved.apiKey ? "set" : "none" }),
  });
